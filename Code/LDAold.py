...
def LDA_algorithm(corpus, V, k):
  ...
  while True:
    ...
    alpha_sum = loggamma(np.sum(alpha_old)) - np.sum(loggamma(alpha_old))

    for (d,document) in enumerate(corpus):
      phi_d, gamma_d = VI_algorithm_new2(k, document, phi2_old[d], gamma2_old[d], lambda2_old, alpha_old, beta_old, eta_old, alpha_sum)
      phi2_new.append(phi_d); gamma2_new.append(gamma_d)
    ...
    # start_l = time.time()
    # print(lower_bound(alpha_old, beta_old, phi2_old, gamma2_old, k, V, corpus))
    # stop_l = time.time()
    # print('...old completed in:', stop_l - start_l)

    start_n = time.time()
    print(lower_bound_corpus(alpha_old, beta_old, phi2_old, gamma2_old, alpha_sum, k, corpus))
    stop_n = time.time()
    print('...new completed in:', stop_n - start_n)
