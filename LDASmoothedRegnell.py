...
def LDA_algorithm(corpus, V, k, tolerance = 1e-4):
  ...
  while True:
    ...
    print("The lower bound changed: {}%\n".format(np.round(100*delta_lower_bound, 5)))
    print("-----------------------------------------------------")
...
def print_predictive_perplexity(alpha, eta, lambdas, test, test_removed, V):
  ...
  for d in range(M):
    phi_d, gamma_d = VI_algorithm(test[d], k, V, lambdas, alpha, eta, gamma_old[d])
    phi_new.append(phi_d); gamma_new.append(gamma_d)
  
  perplexity = predictive_perplexity(beta, gamma_new, test_removed)
...
def main_Reuters():
  ...
  num_topics = k # The number of topics that should be printed
...
def main_each_movie():
  ...
  # Run the algorithm
  [alpha, eta, lambdas, phi, gamma] = LDA_algorithm(training_data, V, k, 1e-4)
