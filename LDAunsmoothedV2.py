...
def VI_algorithm(k, document, phi_old, gamma_old, lambda_old, alpha, beta, eta, alpha_sum, tolerance = 1e-6, debug = False):
...
## OLD VI-algorithm run during the E-step for every document m
def VI_algorithm2(k, document, phi_old, gamma_old, lambda_old, alpha, beta, eta, alpha_sum, tolerance = 1e-4, debug = False):
  lower_bound_old = lower_bound_single(alpha, beta, phi_old, gamma_old, alpha_sum, k, document)
  # if debug:
  #   print(lower_bound_old)
...
## Printing top topics for an article and the likely words from these
def print_article(vocab_file, alpha, beta, test, k, num_articles = 10):
  ...
  for index in range(num_articles):
    ...
    topic_probabilities = gamma / np.sum(gamma)

    phi = phis[index]

    article_topics = np.where(gamma - alpha > 1)[0]

    print("\nThis article 'consists' of ", str(len(article_topics)), " of  the total", len(alpha), "topics")
    print("The topics are:", article_topics, "...")
    print("..with probabilities", topic_probabilities[article_topics])

    print("\nWords with high probability to belonging to a certain topic:")

    word_topics = np.ones(len(document)) * -1 # -1 if none of the topics

    for w in range(len(document)):
      for topic in article_topics:
        if phi[w, topic] > 0.9: # High probability that the word has the topic
          word_topics[w] = topic
                  
    # Get most probable words for each topic
    for (w,word_index) in enumerate(document):
      word = index_word_vocab[word_index + 1] # + 1 since the dictionary starts at 1 and the word_indices in LDA starts at 0
      if word_topics[w] == -1:
        pass
        # print(word, ":: Topic probability too low")   
      else:
...
def main_Reuters():
  ...
  alpha = parameters[0]
  num_topics = 10 # The number of topics that should be printed
  topic_indices = print_likely_topics(alpha, num_topics)
  beta = parameters[1]
  print_top_words_for_all_topics(vocab_file, beta, top_x=15, k=k, indices = list(range(k)))
